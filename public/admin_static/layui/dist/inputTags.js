...
layui.define(['jquery','layer'],function(exports){
  ...
  Class.prototype.init = function(){
    ...
    spantext = $(span).text("获取全部数据").addClass('sc-input-tags-albtn');
    if(options.aldaBtn){
    ...
    options.elem.keypress(function(event){  
      ...
      if(keynum == '13'){  
        ...
        if(options.content.indexOf($val) == -1){
          ...
          that.render()
          spans ='<span><em>'+$val+'</em><button type="button" class="close"><i class="layui-icon layui-icon-close"></i></button></span>';
          options.elem.before(spans)
        }
  ...
  var change = (options)=>{
      options.change && typeof options.change === 'function' && options.change(options.content);
  }
  ...
  Class.prototype.events = function(){
     var that = this
    ,options = that.config;
    $('.sc-input-tags-albtn').on('click',function(){
      console.log(options.content)
    })
    $('.sc-input-tags').on('click','.close',function(){
...
}).link(layui.cache.base + 'inputTags.css')